/*
  ...
  - Added the required column `typeClass` to the `table_history_fly` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "table_history_fly" ADD COLUMN     "typeClass" INTEGER NOT NULL;

-- DropTable
DROP TABLE "_table_history_flyTotable_type_class";

-- AddForeignKey
ALTER TABLE "table_history_fly" ADD CONSTRAINT "table_history_fly_typeClass_fkey" FOREIGN KEY ("typeClass") REFERENCES "table_type_class"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
